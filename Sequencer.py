...
global address
address = id(temp)
print("Check", ctypes.cast(address, ctypes.py_object).value)
...
def set_single_state(nodemap, sequence_number, width_to_set, height_to_set, exposure_time_to_set, gain_to_set):
    ...
    try:
        ...
        node_sequencer_trigger_source = PySpin.CEnumerationPtr(nodemap.GetNode('SequencerTriggerSource'))
        print("HERE###########################")
        sequencer_trigger_source_frame_start = node_sequencer_trigger_source.GetEntryByName('Software')
        ...
        if sequence_number == final_sequence_index:
            ...
            node_sequencer_set_next.SetValue(ctypes.cast(address, ctypes.py_object).value)
    ...
    try:
        ...
        for i in range(NUM_IMAGES):
            try:
                ...
                print((ctypes.c_char).from_address(address))
                if i == 6:
